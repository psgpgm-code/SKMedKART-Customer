...
const normCat=v=>{const s=String(v||'').trim().toLowerCase();if(['human medicines','human medicine','medicine','medicines'].includes(s))return 'Human Medicines';if(['veterinary','veterinary medicines','vet'].includes(s))return 'Veterinary';if(['cosmetics','cosmetic','beauty'].includes(s))return 'Cosmetics';if(['health','health & wellness','health and wellness'].includes(s))return 'Health';if(['baby care','baby'].includes(s))return 'Baby Care';if(['devices','medical devices','device'].includes(s))return 'Devices';return String(v||'').trim()};
const normalizeProduct=(d,id)=>({id,name:d.name||d.productName||'Unnamed Product',cat:normCat(d.cat||d.category),price:Number(d.price??d.sellingPrice??0)||0,stock:Number(d.stock??d.quantity??0)||0,rx:!!(d.rx??d.prescriptionRequired),icon:d.icon||'💊',active:d.active!==false,...d, id});
...
function showNotice(msg,type='warning'){const n=document.getElementById('backendNotice');if(!n)return;if(!msg){n.classList.add('hidden');return}n.classList.remove('hidden');n.className='card '+type;n.innerHTML=msg}
function initProductSync(){
 if(!configured){initLocalProducts();renderProducts();showNotice('<b>📱 Test mode</b><br><span class="small">Firebase is not configured.</span>');return}
 ...
 unsubProducts=onSnapshot(collection(db,'products'),s=>{
   ...
   renderProducts();showNotice('');
 },e=>{
   products=[];renderProducts();
   showNotice('<b>⚠️ Products could not be loaded from Firebase.</b><br><span class="small">'+esc(e.message||'Check Firestore rules.')+'</span>');console.error(e);
 });
}
async function loadProducts(){if(configured)initProductSync();else initProductSync()}
...
window.filterCat=c=>{currentCat=normCat(c);document.getElementById('catTitle').textContent=currentCat+' Catalogue';page('catalogue')};
window.renderProducts=()=>{
 const q=((document.getElementById('search')?.value||'')+' '+(document.getElementById('catSearch')?.value||'')).toLowerCase().trim();
 const arr=products.filter(p=>(currentCat==='All'||normCat(p.cat)===currentCat)&&(!q||[p.name,p.cat].join(' ').toLowerCase().includes(q))&&Number(p.stock||0)>0);
 const html=arr.map(p=>`<div class="card product"><div class="pic">${esc(p.icon||'💊')}</div><div class="info"><b>${esc(p.name)}</b><div class="small">${esc(normCat(p.cat))}</div>${p.rx?'<span class="badge rx">Prescription required</span>':''}<div class="price">${Number(p.price)>0?'₹'+Number(p.price):'Price on confirmation'}</div><div class="small">In stock: ${Number(p.stock||0)}</div></div><button onclick="addCart('${esc(p.id)}')">Add</button></div>`).join('')||'<div class="card small">No products available in this category right now.</div>';
 const home=document.getElementById('products'),cat=document.getElementById('catalogueProducts');if(home)home.innerHTML=currentCat==='All'?html:'';if(cat)cat.innerHTML=html;
};

function cart(){return get('cart',[])}function saveCart(c){set('cart',c);updateCartBar()}
window.addCart=id=>{const p=getProduct(id);if(!p||Number(p.stock||0)<=0)return alert('This product is currently unavailable.');let c=cart(),x=c.find(z=>z.id===p.id);if(x){if(x.qty>=Number(p.stock))return alert('Only '+p.stock+' available.');x.qty++}else c.push({id:p.id,name:p.name,cat:normCat(p.cat),price:Number(p.price)||0,rx:!!p.rx,icon:p.icon,qty:1});saveCart(c);alert(p.name+' added to cart.');};
...
window.changeQty=(i,d)=>{let c=cart(),p=getProduct(c[i].id),max=Number(p?.stock||c[i].qty);c[i].qty=Math.min(max,c[i].qty+d);if(c[i].qty<1)c.splice(i,1);saveCart(c);renderCart()};window.removeCart=i=>{let c=cart();c.splice(i,1);saveCart(c);renderCart()};
...
window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredPrompt=e;const b=document.getElementById('installBtn');if(b)b.classList.remove('hidden')});window.installApp=()=>{if(deferredPrompt){deferredPrompt.prompt();deferredPrompt.userChoice.then(()=>deferredPrompt=null)}else alert('Use Chrome ⋮ → Install app or Add to Home screen.')};
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js'));
loadProducts();renderCart();updateCartBar();
